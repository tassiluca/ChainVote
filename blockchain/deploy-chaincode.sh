...
if [[ $# != 3 || ($1 != "org1" && $1 != "org2") || ! -d $2 ]]; then
    echo "Usage: ./deploy-chaincode <organization-name> <chaincode-path> <chaincode-name>"
    echo "Description:"
    echo "  Script used to deploy chaincode to a Hyperledger Fabric network."
    echo
    echo "Options:"
    echo "  <organization-name>  - The name of the organization, i.e. [org1|org2]"
    echo "  <chaincode-path> - The path to the chaincode executable"
    echo "  <chaincode-name> - The name of the chaincode to deploy"
    exit 1
fi;
